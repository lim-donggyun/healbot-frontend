...
import { searchSymptomsWithAI, searchDiseases, getPopularDiseases } from "../utils/api";
              ...
              <div className="results-summary">
                <div className="summary-text">
                  {searchedSymptoms.length > 0
                    ? `총 ${diseaseList.length}개의 질병이 검색되었습니다.`
                    : "많이 찾는 질환"
                  }
                </div>
                <button
                  ...
                  {searchedSymptoms.length > 0 ? "재검색" : "검색"}
                </button>
              </div>

              {/* 질병 그리드 */}
              <div className="disease-grid">
                {diseaseList.filter(disease => disease != null).map((disease, index) => {
                  // 한국어 필드명 매핑
                  ...
                  return (
                    <div
                      ...
                      <div className="disease-content">
                        ...
                        {department && <p className="department-text">{department}</p>}

                        <div className="disease-symptoms">
                          {symptomsArray.slice(0, 3).map((symptom, idx) => (
                            <span key={idx} className="symptom-badge">
                              {symptom}
                            </span>
                          ))}
                          {symptomsArray.length > 3 && <span className="symptom-more">+{symptomsArray.length - 3}</span>}
                        </div>

                        {description && <p className="disease-description">{description.substring(0, 100)}...</p>}
              ...
              <div className="detail-content-grid">
                ...
                <div className="detail-right">
                  <div className="detail-section">
                    ...
                    <p className="detail-text">{selectedDisease.질환명 || selectedDisease.disease_name || "정보 없음"}</p>
                  </div>
                    ...
                    <div className="detail-section">
                      ...
                      <div className="detail-score-container">
                        <div className="detail-score-bar">
                          <div className="score-bar" style={{ width: `${(selectedDisease.일치도 || selectedDisease.score) * 10}%` }}>
                            <span className="detail-score-text">{(selectedDisease.일치도 || selectedDisease.score).toFixed(1)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
